# Remove unused imports! Can't remember what extension I used but you can find
import pandas as pd
from os import path
...
def main():
    ...
    ## Load data
    # NRPD
    # Where did this come from? Need to explicate lineage either in comments or code
    ...
    # Jodie indexes on the first two increments, not just the first. Shouldn't we be doing that if we're basing on August signups?
    ...
    # nov pcp per signup
    # This logic here is inappropriate, we should be taking 2% of the forecasted NRPD in total and applying on a bookings basis. For sake of time we can leave but something we likely need to update.
    nov["pcp_per_signup"] = nov["projected_paid_days"] * (
        nov["nrpd_forecast_v2"] * 0.98 - nov["cost_per_day"]
    )

    # Redundant code (should just be 
    # `nov['pcp_per_signup_with_halo'] = nov['pcp_per_signup'] * nov['paid_halo']`)
    ...
    # What's the function of this? Seems like this would achieve our weighted average for Nov signup NRPD, why are we not using nrpd_forecast_v2 directly?
